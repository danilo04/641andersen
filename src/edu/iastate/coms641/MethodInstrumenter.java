...
public class MethodInstrumenter extends BodyTransformer {
	...
	protected void internalTransform(Body body, String phaseName,
		...
		// we dont want instrument our helper class
		...
		// obtain the name of the method <classname.methodname>
		// we can use signature here for more precision
		...
		while (stmtIt.hasNext()) {
			...
			if ((stmt instanceof ReturnStmt)
				...
				// make invoke expression of MyCounter.report()
				if (isMain) {
					InvokeExpr reportExpr = Jimple.v().newStaticInvokeExpr(
							reportCounter.makeRef());
					// then, make a invoke statement
					Stmt reportStmt = Jimple.v().newInvokeStmt(reportExpr);

					// insert new statement into the chain
		...
		//validate that our changes are correct and we did not mess up
		body.validate();
	}
